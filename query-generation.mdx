...
To change queries on an experiment that already exists, open **Settings → Queries** and click **Edit queries**. That opens the linked set here so you can add, edit, or remove queries. Changes apply to the next run; earlier results stay as they were. If the experiment is not linked to a set yet, **Edit queries** creates one from its current queries.
