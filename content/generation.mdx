...
<Steps>
  <Step title="Choose a content type">
    ...
    | `pdf` | Reports, whitepapers, or other document-style content intended to be shared as a PDF |
  ...
  <Step title="Optionally add links">
    Two toggles control extra linking during generation:

    - **Add backlinks to content** — insert third-party authoritative source links.
    - **Add internal links to content** — insert links to relevant pages on your company domain from the indexed sitemap. This option is available once a company domain is set.
  </Step>
  ...
  <Step title="Generate and review sections">
    Click **Generate Content**. For most content types, XLR8.ai runs an agentic workflow and shows **Generation progress** instead of a spinner — pipeline steps (draft, optional links, compliance report, citations, and save), an activity log, self-review notes, and sources applied so far.

    Agentic runs can take up to about 15 minutes. Keep the tab open; XLR8.ai keeps polling until the draft is ready. If you open the piece from the library while it is still running, you see the same progress view.
...
If your organization has configured **Organization Brand Guidelines** in Settings, content generation can include them by default. You can also toggle **Use org brand guidelines** on the generation form. When the toggle is on, each guidelines text and uploaded file shows a short preview of the uploaded text so you can choose what to include.

Approved **AI Brand Learnings** from comment threads further steer future content. Use **Remember** in the Comments sidebar to save selected threads for review in Settings.

## Comments and review

On the content detail view, the **Comments** sidebar lists comment and suggestion threads on the generated piece. A **Suggested changes** tab sits next to Comments for brand-correction diffs.
...
- Pending suggestions also offer **Accept** and **Reject**. Accepting a suggestion applies the edit and marks the thread as acted on.
- When the status filter is **Open**, click **Resolve All** to mark every open thread as acted on. The confirmation notes that this does not add brand learnings. Thread actions are disabled while bulk resolve is in progress.
- Check the threads you want to reuse, then click **Remember**. The button shows how many threads are selected. Threads already saved as a brand learning cannot be selected again.
- Saved threads show a status badge under the title: **Pending**, **In learnings**, or **Dismissed**.

Selected threads become **AI Brand Learnings** in [Settings](/integrations/settings) for review. Threads that are already saved or not reusable are skipped.

## Reviewing a generated draft

After generation finishes, the detail view can include:

- **Compliance report** — expand it to see whether the draft passed, is blocked, or has issues. Issues are grouped by category and can include a short fix hint. Prompt contradictions appear separately when the draft conflicts with your instructions.
- **Generation trace** — expand it to read the agent activity log and self-review notes. Long notes have **View full note**.
- **Brand learning** or **Brand guideline** badges on sections — hover or click to see which approved learnings or guidelines shaped that section.

## Suggested changes

Use brand-correction feedback to update a generated piece without rewriting it by hand.

- Click **Regenerate with feedback** in the toolbar to apply approved brand learnings across the piece.
- Or click the sparkle button on a section to regenerate only that section.
- Review the result on the **Suggested changes** tab, and **Accept** or **Reject** each section. Suggested diffs also appear inline on the changed sections.
- If you leave the page before you finish reviewing, your pending suggested changes are saved and restored when you return.

## Download and export

From the content editor, click the download button to export the full piece as **Markdown**, **HTML**, or **PDF**. The PDF uses your current section text — including any optimized versions you selected — and saves a letter-sized file named after the content title.
...
- Content type and the exact LLM version used (for example, Claude Opus 4.7)
- Creation date
