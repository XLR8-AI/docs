...
XLR8.ai's domain analyzer suggests categories and queries automatically based on your company domain and experiment type. You build and edit this map in [Query Generation](/query-generation), then pick the saved query set when you create an experiment. On an existing experiment, organization admins can also open **Settings → Queries** and click **Edit queries** to change the linked set.

## Query sets

A **query set** is a reusable briefing plus category map. It stores experiment type and subtype, company name, domain, aliases, tracked competitors, categories, queries, and optional Keyword Planner keywords with language and location targeting.
...
Domain analysis in [Query Generation](/query-generation) suggests aliases from your website. You can add or remove them on the Categories step. They are saved on the query set and copied onto new experiments.
