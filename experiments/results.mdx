...
If you've configured [tracked URLs](/experiments/visibility#tracked-urls), Source Analytics includes **Tracked URLs impact** — citation counts and whether your brand was mentioned on those citing prompts. When earlier successful runs exist, choose **Previous execution** or **First execution** as the baseline. Those comparisons load after the rest of the page; the heading stays visible while they do. The first successful run still shows the table, without baseline buttons. **Avg previous** is not available here — use [Analytics → Sources](/analytics/overview) when you want an average of prior runs.
...
- **Detailed Analysis** — category-level sentiment, pros/cons, and supporting citations. Filter rows by polarity: **Positive (3.0 to 5.0)**, **Neutral (0.0 to less than 3.0)**, or **Negative (below 0.0)**. On the **Pros** and **Cons** columns, use the company-name / **Competitor** toggle (defaults to your company name) to show phrases about your brand or phrases attributed to other brands. Competitor view labels each phrase with the brand. **Export to CSV** uses the same filter and records the selected mode. Phrases without a brand attribution are hidden in both views.

If the experiment has tracked URLs, **Detailed Analysis** shows **Tracked URLs sentiment** — mean sentiment on queries that cite those URLs. Compare change to **Previous execution** or **First execution** the same way as visibility Source Analytics. **Avg previous** is on [Analytics → Sources](/analytics/overview), not on the experiment page.
