...
<Steps>
  <Step title="Create a query set (if you do not have one)">
    From the sidebar under **Strategy**, click **Query Generation**, then **New query set**. Enter company details, analyze the domain, review categories, aliases, and tracked competitors, generate or import queries, and **Save query set**.
  ...
  <Step title="Step 1 — Query set">
    Pick a saved query set that already has queries. Experiment type, company, domain, aliases, and tracked competitors come from that set — you will not enter them again.

    The selector shows each set's name, company, category count, and query count. After you select one, a summary shows company, domain, type/subtype, aliases, tracked competitors, and query counts. Use **Edit** to open the set in Query Generation, or **Manage query sets** to go to the list.
...
To re-run a single model on the current execution instead of starting a new run, use **Settings → Execution**.

### Experiment settings

Open **Settings** on an experiment to manage:

- **Execution** — per-model status for the selected run (**Succeeded**, **Failed**, **In progress**, or **Auto-retry**). Organization admins can click **Re-run** on a model, or **Re-run failed** when more than one failed. That writes into the same execution, and the row shows **In progress** or **Auto-retry** while it runs — even if the overall execution already completed.
...
- **Queries** — the **Queries** tab lists every query by category. Organization admins can click **Edit queries** to open the linked set in [Query Generation](/query-generation), where you can add, edit, or remove queries. Changes apply to the next run; earlier execution results stay as they were. If the experiment is not linked to a query set yet, **Edit queries** creates one from the current queries.
- Categories
