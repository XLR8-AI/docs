...
You choose the sentiment subtype and generate or import queries in [Query Generation](/query-generation), then pick that query set when you create the experiment. Domain analysis suggests company aliases and tracked competitors, which are saved with the set and copied onto the experiment.
...
**Pros and cons** are the most actionable outputs. Start with your company name to focus on your brand, then switch to **Competitor** when you want to see how models talk about others. Review the cons list to identify specific criticisms that AI models consistently surface — these represent content and messaging gaps you can address directly. To see how similar phrases group together, open **Reporting → Analytics** and use the **Themes** tab. The theme cluster map starts in 3D so you can rotate clusters; switch to 2D to pan and zoom. Filter the cluster map by aspect, modifier, and intensity for the current Pros or Cons polarity.

**Citations** tell you which sources the AI model used to form its response. If the sources cited are competitor content, third-party reviews, or outdated pages about your brand, you have a clear signal about where to focus your content strategy. When tracked URLs are configured, **Detailed Analysis** also shows **Tracked URLs sentiment**, with change vs **Previous execution** or **First execution**. See [Reading results](/experiments/results).
