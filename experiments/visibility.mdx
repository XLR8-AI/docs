...
When you create an experiment from a query set, aliases inferred during domain analysis — plus any you added on the Categories step — are copied onto the experiment.

**To review or add aliases:**
...
**Tracked competitors** are the brands XLR8.ai watches for mention rate alongside yours.

Domain analysis in [Query Generation](/query-generation) infers competitors for every subtype. For **Competitor alternatives**, the names also match the competitor list on that step. Creating an experiment from the query set saves those names on the experiment.

You can still add or edit them in **Settings → Competitors**. Saving competitor names recomputes mention rate from existing results — you do not need to re-run the experiment.
...
When you configure tracked URLs, XLR8.ai identifies which citations in AI responses match your URLs and computes a **tracked URL impact** score that quantifies how much citation traffic those pages are generating across your queries. On **Source Analytics**, compare citation change to **Previous execution** or **First execution**. Those comparisons load after the rest of the results page. See [Reading results](/experiments/results).
